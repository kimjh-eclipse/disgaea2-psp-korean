...
v20260904부터 빠른패처는 기존 암호화 세이브의 암흑의회 의원명도 직접
한국어화합니다. `DATA.BIN`과 같은 폴더의 `PARAM.SFO`를 함께 읽어 ULJS00183의
mode-5 세이브를 복호화하고, 64개 의원 레코드가 전부 일치할 때만 이름 필드를
치환한 뒤 재암호화합니다. 재복호화 왕복 검증을 통과해야 원본을 교체하며 두 파일
모두 `.d2assemblybak`으로 백업합니다. 패처 폴더에는
`D2_SAVE_assemblymap.bin`과 `D2_SAVE_crypto.exe`가 추가로 필요합니다.
...
v20260831까지는 Apache-2.0이었습니다. PSP 세이브 암복호 구현을 참조·이식하기 위해
GPL-3.0으로 바꿨습니다 — 참조 구현([psp-save](https://github.com/vita8328/psp-save)의
`chnnlsv.c`)이 **GPL-2.0-or-later**이고, GPL-2.0(only)은 Apache-2.0과 양립하지 않지만
GPL-3.0은 GPL-2.0-or-later 코드를 받아들이면서 기존 Apache-2.0 부분과도 양립합니다.

- 기여자는 저작권자 본인 단독이라 재라이선스에 제3자 동의 문제는 없습니다.
- **이미 배포된 v20260831 이하 릴리즈는 당시 라이선스(Apache-2.0)를 그대로 따릅니다.**
  라이선스 변경은 소급되지 않습니다.
- 이 저장소를 기반으로 배포물을 만들 경우 GPL-3.0에 따라 소스를 공개해야 합니다.

게임 데이터는 저작물이므로 이 라이선스의 적용 대상이 아니며 저장소에 포함하지 않습니다.
